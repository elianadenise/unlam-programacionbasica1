...
public class Comision {
	...
	private int id;//DONDE LO USO?
	private String materia;
	private char turno; // M -> Mañana | N -> Noche -- DONDE VRGA LO USO?
	...
	public Comision() {
		...
		this.personas = new Persona[this.CANTIDAD_PERSONAS];
	}
	...
	public boolean ingresarPersona(Persona persona) {
		...
		Persona existente = this.buscarPorDni(persona.getDni());
		boolean pudoIngresar = false;
		for(int i = 0; i < personas.length; i++) {
			if(personas[i] == null && existente == null) {
				personas[i] = persona;
				pudoIngresar = true;
				break;
			}
	...
	public boolean recuperarNota(Integer dni, Nota nota) {
		...
		/*for(int i = 0; i < personas.length; i++) {
			if(personas[i] != null && persona != null) {
				if(personas[i].getDni().equals(persona.getDni())) {
					persona = personas[i];
				}
	...
	public Persona[] obtenerPersonasDeLaCarreraQuePromocionaron(Carrera carrera) {
		...
		Persona listado[] = new Persona[personas.length];
		int indice = 0;
		for(int i = 0; i < personas.length; i++) {
			if(personas[i] != null) {
				if(personas[i].getCarrera().equals(carrera) && personas[i].obtenerCondicionFinal().equals(CondicionFinal.PROMOCIONA)) {
					if(listado[indice] == null) {
						listado[indice] = personas[i];
					}
				}
			}
			indice++; //para mi tiene que estar dentro del segundo if
		}
	...
	public Double obtenerElPromedioDeNota2DeLasPersonasQueCursaron() {
		...
		for(int i = 0; i < personas.length; i++) {
			if(personas[i] != null) {
				sumatoria += personas[i].getParcial2().getValor();
				divisor++;
			}
		}
